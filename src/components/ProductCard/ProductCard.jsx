...
const ProductCard = ({ product }) => {
    ...
    return (
        <div className="group bg-white rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden border border-gray-100 flex flex-col h-full relative">
            <Link to={`/product/${product.id}`} className="block h-full cursor-pointer">
                ...
                <div className="relative aspect-w-4 aspect-h-3 overflow-hidden bg-gray-100 h-64 sm:h-72">
                    ...
                    <div className="absolute top-3 left-3 flex flex-col gap-2 z-10">
                        {product.inStock && (product.isSale || (product.originalPrice > product.price)) && (
                    ...
                    <div className="absolute bottom-0 left-0 w-full p-4 translate-y-full group-hover:translate-y-0 transition-transform duration-300 z-20 pointer-events-none group-hover:pointer-events-auto">
                        <button
                            onClick={(e) => {
                                e.preventDefault();
                                e.stopPropagation();
                                if (addToCart && product.inStock) addToCart(product);
                            }}
            ...
            {/* Hover Actions - Right Side (Quick View & Wishlist) - ABSOLUTE POSITIONED ON TOP OF LINK */}
            <div className="absolute top-3 right-3 flex flex-col gap-2 translate-x-12 opacity-0 group-hover:translate-x-0 group-hover:opacity-100 transition-all duration-300 z-20 pointer-events-none group-hover:pointer-events-auto">
                <button
                    onClick={(e) => { e.preventDefault(); e.stopPropagation(); /* Add Quick View Logic */ }}
